...
def get_selected_joint_names(*arg):
    selected_joints = cmds.ls(selection=True, type='joint')
    if selected_joints:
        joint_names = ', '.join(selected_joints)
        cmds.textField("select_jnt_viewport", edit=True, text=joint_names)

def create_circles_for_joints(*arg):
    selected_joints = cmds.ls(selection=True, type='joint')
    value_Ctrl = float(cmds.textField("scale_value_Ctrl", query=True, text=True))
    if selected_joints:
        for joint in selected_joints:
            group_ctrl = cmds.group(n=joint + "_Grp", empty=True)
            circle_out_jnt = cmds.curve(name=joint + '_Ctrl', degree = 1,\
            ...
            cmds.xform(group_ctrl, worldSpace=True, translation=joint_position)
            cmds.xform(circle_out_jnt, worldSpace=True, translation=joint_position)
            cmds.scale(value_Ctrl, value_Ctrl, value_Ctrl)
            cmds.parent(circle_out_jnt, group_ctrl)
            cmds.makeIdentity(circle_out_jnt, apply=True)
            cmds.parent(joint, circle_out_jnt)
...
################################## IK FK SW
    # (0, 0, 0),(-2, 0, 0),(-2.01, 0, 0.16),(-2.05, 0, 0.31),
    # (-2.11, 0, 0.46),(-2.19, 0, 0.59),(-2.29, 0, 0.71),
    # (-2.41, 0, 0.81),(-2.55, 0, 0.89),(-2.69, 0, 0.95),
    # (-2.84, 0, 0.99),(-3, 0, 1),(-3.16, 0, 0.99),(-3.31, 0, 0.95),
    # (-3.45, 0, 0.89),(-3.59, 0, 0.81),(-3.71, 0, 0.71),
    # (-3.81, 0, 0.59),(-3.89, 0, 0.46),(-3.95, 0, 0.31),
    # (-3.99, 0, 0.16),(-4, 0, 0),(-3.99, 0, -0.16),(-3.95, 0, -0.31),
    # (-3.89, 0, -0.46),(-3.81, 0, -0.59),(-3.71, 0, -0.71),
    # (-3.59, 0, -0.81),(-3.45, 0, -0.89),(-3.31, 0, -0.95),
    # (-3.16, 0, -0.99),(-3, 0, -1),(-2.84, 0, -0.99),(-2.69, 0, -0.95),
    # (-2.55, 0, -0.89),(-2.41, 0, -0.81),(-2.29, 0, -0.71),
    # (-2.19, 0, -0.59),(-2.11, 0, -0.46),(-2.05, 0, -0.31),
    # (-2.05, 0, -0.31),(-2.01, 0, -0.16),(0, 0, 0)
    # (0, 0, 0),(2, 0, 0),(2.01, -0.16, 0),(2.05, -0.31, 0),
    # (2.11, -0.46, 0),(2.19, -0.59, 0),(2.29, -0.71, 0),(2.41, -0.81, 0),(2.55, -0.89, 0),
    # (2.69, -0.95, 0),(2.84, -0.99, 0),(3, -1, 0),(3.16, -0.99, 0),(3.31, -0.95, 0),
    # (3.45, -0.89, 0),(3.59, -0.81, 0),(3.71, -0.71, 0),(3.81, -0.59, 0),
    # (3.89, -0.46, 0),(3.95, -0.31, 0),(3.99, -0.16, 0),(4, 0, 0),(3.99, 0.16, 0),
    # (3.95, 0.31, 0),(3.89, 0.46, 0),(3.81, 0.59, 0),(3.71, 0.71, 0),(3.59, 0.81, 0),
    # (3.45, 0.89, 0),(3.31, 0.95, 0),(3.16, 0.99, 0),(3, 1, 0),(2.84, 0.99, 0),
    # (2.69, 0.95, 0),(2.55, 0.89, 0),(2.41, 0.81, 0),(2.29, 0.71, 0),(2.19, 0.59, 0),
    # (2.11, 0.46, 0),(2.05, 0.31, 0),(2.05, 0.31, 0),(2.01, 0.16, 0),(2, 0, 0)
...
# createNode joint -n "IK_Spine" -p "IK_Root";
...
# createNode joint -n "IK_Chest" -p "IK_Spine";
