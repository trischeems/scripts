...
def swShadermtl(*arg):
    ...
    connectAttr -f VRayMtl1.outColor VRayMtl1SG.surfaceShader;
             """)
    cmds.setAttr("VRayMtl1.useRoughness", True)
    VRayMtl = "VRayMtl1"
    AttrMtl ="VRayMtl1.reflectionColor"
    AttrMtl_vl = (0.5, 0.5, 0.5)
    if cmds.objExists(VRayMtl) and cmds.attributeQuery("bumpMapType", node=VRayMtl, exists=True):
        cmds.setAttr(VRayMtl + ".bumpMapType", 1)
    cmds.setAttr(AttrMtl, AttrMtl_vl[0], AttrMtl_vl[1], AttrMtl_vl[2], type="double3")
    ...
    //setAttr "Text1.ignoreColorSpaceFileRules" 1;             
             """)
    cmds.rename(file, "normal")
    cmds.rename(place, "Normal_Place2d")
    cmds.setAttr('normal.ignoreColorSpaceFileRules', True)
    # cmds.setAttr('normal.colorSpace', "raw", type="String")
    cmds.connectAttr('normal.outColor', 'VRayMtl1.bumpMap')

    NormalName = "normal"
    if cmds.objExists(NormalName) and cmds.attributeQuery("colorSpace", node=NormalName, exists=True):
    ...
    //setAttr "Text1.ignoreColorSpaceFileRules" 1;             
             """)
    cmds.rename(file, "roughness")
    cmds.rename(place, "Roughness_Place2d")
    cmds.setAttr('roughness.ignoreColorSpaceFileRules', True)
    # cmds.setAttr('roughness.colorSpace', "raw", type="String")
    cmds.connectAttr(' roughness.outAlpha', 'VRayMtl1.reflectionGlossiness')

    RoughnessName = "roughness"
    if cmds.objExists(RoughnessName) and cmds.attributeQuery("colorSpace", node=RoughnessName, exists=True):
...
def mtl_base(*arg):
    ...
    VRayMtl = "VRayMtl1"
    AttrMtl ="VRayMtl1.reflectionColor"
    AttrMtl_vl = (0.51049, 0.51049, 0.51049)
    if cmds.objExists(VRayMtl) and cmds.attributeQuery("bumpMapType", node=VRayMtl, exists=True):
        cmds.setAttr(VRayMtl + ".bumpMapType", 1)
    cmds.setAttr(AttrMtl, AttrMtl_vl[0], AttrMtl_vl[1], AttrMtl_vl[2], type="double3")
    ...
    cmds.rename(file, "baseColor")
    cmds.rename(place, "BaseColor_Place2d")
    cmds.setAttr('baseColor.ignoreColorSpaceFileRules', True)
    cmds.connectAttr('baseColor.outColor', 'VRayMtl1.color')
